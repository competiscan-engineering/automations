...
def catalog() -> dict:
    ...
    with _CATALOG_LOCK:
        try:
            return CS.catalog(max_age=86_400)
...
def _sync_filters(section: dict) -> None:
    ...
    have = list(q.get("filters") or [])
    for key in (q.get("enhanced") or {}):
